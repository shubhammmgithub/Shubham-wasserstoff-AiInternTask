...
initailly i started with openai but due to limited search . 
...
# handles file upload,it will take pdf or image ,
# performs ocr,chunks tet and generate embeddings and update FAISS index and stores metadata.
...
"""
 ...
 Extract common themes from uploaded documents:
    Groups chunks by document
     Limits to 20–30 chunks per doc to avoid token limit
    Sends prompt to LLM to generate summarized themes
...
def extract_themes():
    ...
    for doc_id, chunks in grouped.items():
        # It will Limit number of chunks per doc to avoid token overflow that was causing the error i.e groq was unable to take that much in less time
        limited_chunks = chunks[:30]  # we can tune this number according to our preferences
